...
<div class="settWrap applyAdvertsWrap ">
	...
	<div>
		<a class="tabItm active" href="/i/administracion/publicidad">Artículos destacados</a>
	...
	<form id="addPubForm" class="displayAdvertForm" method="post" action="/i/administracion/pub_func/add_item_pub.php">
				...
				<div class="featItemDescript <?php echo ($oddCount % 2) == 0 ? "" : "odd"; $oddCount++; ?>">	
					...
					<span class="featItemPrice">
						...
						if(($dia <= 0 and $horas <= 0) or ($producto->terminado == 1))
						{
							...
							<form method="post" action="/i/administracion/pub_func/finish_pub.php">
						...
						?>
						<form method="post" action="/i/administracion/pub_func/remove_pub.php">
								<input type="hidden" name="idproductopub" value="<?php echo $producto->idproductopub ?>">
...
<script>
	$('.itemShow').keyup(function(){
		...
		if(itemId)
		{
			$.ajax({
				type : "POST",
				url  : "/i/administracion/pub_func/preview_item.php",
				data : {id : itemId}
			}).done(function(html){
