...
<div class="topBar">
	...
	<a class="view-report-btn setting-extra-btn" href="/i/administracion/rep/users.php">Reporte de Tiendas</a>
			...
			if($total_pages > 1)
				{
					echo $page == 1 ? 1 : (($page-1)*$page_size)+1 ?> al <?php echo $page == $total_pages ? $total : ($page*$page_size) ?> de <span><?php echo $total; ?></span>
						...
						<a <?php echo $page > 1 ? "href='/i/administracion/usuarios/page/$prevPage/'" : "" ?> class="pageCtrls prevPage <?php echo $page > 1 ? "active" : "inactive" ?>"><i class="fa fa-angle-left transOn"></i></a>
		
		
						<a <?php echo $page < $total_pages ? "href='/i/administracion/usuarios/page/$nextPage/'" : "" ?> class="pageCtrls nextPage <?php echo $page < $total_pages ? "active" : "inactive" ?>"><i class="fa fa-angle-right transOn"></i></a>
...
<div class="itemDetWrapp transTw userDetailWrapper <? if($oddCnt%2==1){ echo "odd";} echo $uA->baneado == 1 ? " inactive" : ""; $oddCnt++  ?>" >
	...
	?>
		<span class="itmImageMask itmListMask usrImageMask">
			<img class="itmListImg itmImage" style="left:-<?php echo $leftPos; ?>px;" width="<?php echo $width; ?>" height="<?php echo $baseDimm; ?>" src="/i/images/profile/cr/<?php echo $uA->imagen ?>">
		</span>
	<?php 
	}
		...
		<span class="itmImageMask usrImageMask transOn itmListMask noImg">
			...
			<img src="/i/images/resources/<?php echo $tipoUsr->idtipousuario == 2 ? "storePNG" : "userPNG" ?>-100.png" alt="No user picture" width="85" height="85" />
	...
	?>
		...
		<div class="itmListBtnWrapp hidden">
			...
			?>
				<a class="itmListBtn viewListBtn" href="/i/administracion/usr/<?php echo $uA->idusuario; ?>">Ver Artículos</a>
...
$('.removeSrchBtn').click(function()
{
	...
	$.ajax({
		type : "POST",
		url : "/i/administracion/usu_func/reset_srh.php"
	}).done(function(){
		location.reload();
	})
})
...
$('.listSearchBtn').click(function()
{
	...
	$.ajax({
		type : "POST",
		url : "/i/administracion/usu_func/set_search.php",
...
<script>
	$('.banButton').click(function(){
		...
		$.ajax({
			type : "POST",
			url : "/i/administracion/usu_func/ban_user.php",
			data : {usr : idusuario}
		})
	})
</script>