...
<div class="settWrap manageCuponsWrap">
	...
	<div class="listActionsWrap listSearchWrap">
		<form method="post" action="/i/administracion/cuponesnuevo">
		...
		foreach($records->data as $c)
		{
			...
			<div class="itemDetWrapp fluidTransTw <? if($oddCnt%2==1){ echo "odd ";} echo $c->estatus == 1 ? "" : "inactive"; echo $c->aprobado == 0 ? "pendiente" : ""; $oddCnt++  ?>">
				<a class="itmImageMask itmListMask"  href="/i/cupon/<?php echo $link . '-' . $c->idcupon;  ?>">
					<?php 
					...
					{
					?>
					<img class="itmListImg itmImage" <?php if($height > $width){  ?> width="<?php echo $baseDimm; ?>" height="<?php echo $modVal; ?>" style="top: -<?php echo $pos; ?>px;"  <?php }  else{ ?> height="<?php echo $baseDimm; ?>" width="<?php echo $modVal; ?>" style="left: -<?php echo $pos; ?>px;" <?php } ?> src="/i/images/cupon/thumb150/<?php echo $img->data[0]->imagen; ?>">
					<?php 
					}
					?>
				</a>
				...
				<div class="itmListDescrWrap">
					<h3><a class="itmListName transOn itmListDescr" href="/i/cupon/<?php echo $link . '-' . $c->idcupon;  ?>"><?php echo $c->titulo ?></a></h3>
				...
				<div class="itmListBtnWrapp">
					...
					<a class="itmListBtn editBtn" href="/i/administracion/cuponesmod/<?php echo $c->idcupon ?>">Modificar</a>
					<?php 
					if($c->aprobado <> 0)
					{
						if($c->estatus == 1)
...
<script>
	$('.itmRemoveBtn').click(function(){
		...
		$.ajax({
			type : 'POST',
			url : '/i/administracion/cup_func/rem_cupon.php',
...
<script>
	...
	$('.aproveBtn').click(function()
	{
		...
		$.ajax({
			type : 'POST',
			url : '/i/administracion/cup_func/aprobado_cup.php',
...
<script> //show/hides item's edit buttons when on mobile
	...
	$('.aprovedWrap').on('click', '.itmStatBtn', function(){
		...
		$.ajax({
			type : "POST",
			url : '/i/administracion/cup_func/status_cup.php',
			data :{idcupon : idc}
		});
