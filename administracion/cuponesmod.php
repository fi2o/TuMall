...
<div class="settWrap updateItemsWrap">
	<div class="couponForms itmFormSec" >
		...
		<form id="cuponForm" method="post" action="/i/administracion/cup_func/mod_cupon.php">
				...
				<div class="previewUser itemPreview">
					...
					if($usr->data[0]->idtipousuario == 2)
					{
						...
						<span class="itmImageMask itmListMask usrImageMask">
							...
							{
							?>
								<img class="userImage itmListImg itmImage" src="/i/images/profile/cr/<?php echo $usr->data[0]->imagen ?>">
							<?php
							}
							else
							{
							?>
								<img class="userImage itmListImg itmImage" src="/i/images/resources/storePNG-100.png">
							<?php
							}
							?>
						</span>
				...
				<div class="previewProduct itemPreview">
					...
					?>
						...
						<span class="pictureWrapper">
							...
							{
							?>
								<img src="/i/images/productos/thumb200/<?php echo $prod->data[0]->imagen ?>" title="<?php echo $prod->data[0]->nombre; ?>" alt="<?php echo $prod->data[0]->nombre; ?>" >
							<?php 
							}
							?>
						</span>
	...
	<div class="couponForms itmFormSec" >
		...
		<form id="cuponGalleryForm" method="post" enctype="multipart/form-data" action="/i/administracion/cup_func/add_image.php">
		...
		<div class="imagenesCupones">
			...
			foreach($imagenCupon->data as $row)
			{
				...
				<div class="couponImagesWrap">
					...
					<br />
					<img class="cuponImage" src="/i/images/cupon/thumb150/<?php echo $row->imagen ?>">
				</div>
				<?php
				$imgCnt++;
			}
	...
	<script src="http://tumall.do/i/Scripts/datepicker/jquery-ui-1.10.4.custom.min.js"></script>
	...
	<script type="text/javascript">
		$('.cuponProduct').keyup(function(){
			...
			if(!id == ""){
					$.ajax({
						type : "POST",
						url : "/i/administracion/cup_func/product_preview",
	...
	<script type="text/javascript">
		$('.cuponUser').keyup(function(){
			...
			$.ajax({
				type : "POST",
				url : "/i/administracion/cup_func/user_preview",
				data : {idusuario : id}
			}).done(function(html)
			{
     ...
     var locations = [    	<?php foreach($pos->data as $p)
      ...
      var markers = [];
		function placeMarker(position, map) {
		    ...
		    position: position,
		    map: map,
		    icon : '/i/images/icono_tumall.png'
		    }); 
    ...
    <script>
    	$('.imagenesCupones').on('click','.idCuponImage',function(){
    		...
    		$.ajax({
    			type : 'POST',
    			url : '/i/administracion/cup_func/prin_image.php',
    ...
    <script>
    	$('.imagenesCupones').on('click','.cuponImage',function(){
    		...
    		$.ajax({
    			type : 'POST',
    			url : '/i/administracion/cup_func/rem_image.php',
