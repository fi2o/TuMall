...
<div class="settWrap applyAdvertsWrap">
	...
	<div>
		<a class="tabItm" href="/i/administracion/publicidad">Artículos destacados</a>
		<a class="tabItm active" href="/i/administracion/tiendas">Tiendas destacadas</a>
		<a class="tabItm" href="/i/administracion/anuncios">Anuncios</a>
	</div>
	
	<form id="addPubForm" class="displayAdvertForm" method="post" action="/i/administracion/pub_func/add_store_pub.php">
			...
			<div class="itemDetWrapp transTw userDetailWrapper <? if($oddCnt%2==1){ echo "odd";} $oddCnt++  ?>" >
				...
				?>
					<span class="itmImageMask itmListMask usrImageMask">
						<img class="itmListImg itmImage" width="85" height="85" src="/i/images/profile/cr/<?php echo $uT->imagen ?>">
					</span>
				<?php 
				}
				else{
					...
					<span class="itmImageMask usrImageMask transOn itmListMask noImg">
						<img src="/i/images/resources/storePNG-100.png" alt="No user picture" width="85" height="85" />				
					</span>
					<?php
				}	
				?>
				...
				$dia = $dias->format('%a');
				$horas = $dias->format('%H');
				if(($dia == 0 and $horas == 0) or ($uT->terminado == 1))
				{
					?>
				...
				<div class="itmListBtnWrapp hidden">
					...
					<form class="endPromoteForm" method="post" action="/i/administracion/pub_func/finish_store_pub.php">
				...
				?>
					...
					<form method="post" action="/i/administracion/pub_func/remove_store_pub.php">
						<input type="hidden" name="idtiendapub" value="<?php echo $uT->idtiendapub ?>">
					</form>
				</div>
			</div>
...
<script>
	$('.storeShow').keyup(function(){
		...
		if(storeId)
		{
			$.ajax({
				type : "POST",
				url  : "/i/administracion/pub_func/preview_store.php",
				data : {id : storeId}
			}).done(function(html){
