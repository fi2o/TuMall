...
<div>
	...
	<form method="post" id="GalleryForm" action="/i/administracion/gal_func/add_image.php" enctype="multipart/form-data">
	...
	<div class="imagenesGaleria">
		...
		foreach($imagen->data as $row)
		{
			...
			<div class="imageGallery">
				<img class="galleryImage" src="/i/images/galeria/thumb150/<?php echo $row->imagen ?>">
...
<script>
	$('.orden').click(function()
	{
		...
		$.ajax({
			type : 'POST',
			url : '/i/administracion/gal_func/mod_ord.php',
...
<script>
	$('.imagenesGaleria').on('click','.removeGal',function(){
		...
		$.ajax({
			type : 'POST',
			url : '/i/administracion/gal_func/remove_image.php',
			data : {idGaleria : id}
		});
	})
</script>
...
<script>
	$('.imagenesGaleria').on('keyup','.url',function(){
		...
		$.ajax({
			type : 'POST',
			url : '/i/administracion/gal_func/mod_url.php',
...
<script>
	$('.imagenesGaleria').on('click','.idgaleria',function(){
		...
		$.ajax({
			type : 'POST',
			url : '/i/administracion/gal_func/change_cup_stat.php',
