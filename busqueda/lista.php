	...
	<div class="resultsWrapper">
		...
		foreach($productos as $p)
		{
			...
			<div class="resultsItemWrapper group <?php echo $total == $item_cont ? "last" : "" ?>">
				<a href="/i/articulo/<?php echo $link."-".$p->idproducto; ?>" class="pictureWrapper">
						<?php 
						if(empty($p->imagen))
						{
						?>
							<img src="/i/images/NoImage.png" alt="No image" title="No image">
						<?php 
						}
						else
						{
						?>
							<img src="/i/images/productos/thumb200/<?php echo $p->imagen ?>" title="<?php echo $p->nombre; ?>" alt="<?php echo $p->nombre; ?>" >
						<?php 
						}
						?>
				</a>
				<div class="resultsItemDescrip">
					<h3 class="resultsItemName"><a href="/i/articulo/<?php echo $link."-".$p->idproducto; ?>"><?php echo $p->nombre; ?></a></h3>
					...
					<div class="resultSellerWrap">
						de <a class="resultSellerName" href="/i/<?php echo $storeLink."-".$p->idusuario."/"; ?>"><?php echo $p->nombretienda; ?></a>
						<?php 
						if($p->estado == 'A')
						{
							?>
			...
			if($total_pages > 1)
				{
					...
					<?php
						...
						<a class="pageControl prevPage" href="/i/lista/
					...
					for($i = 1; $i <= $total_pages; $i++)
					{
						if($page == $i)
						{
							...
							{
								?>
								<a class="toPage pageNumber" href="/i/lista/
								<?php echo empty($_GET["categoria1"]) ? "" : "categoria/$ct1-$cat1->idcategoria1/"  ?>
								<?php echo empty($_GET["categoria2"]) ? "" : "categoria2/$ct2-$cat2->idcategoria2/" ?>
								<?php echo empty($_GET["buscar"]) ? "" : "buscar/$busqueda/" ?>
								<?php echo $i ?>">
					...
					<?php
						...
						<a class="pageControl nextPage" href="/i/lista/
						<?php echo empty($_GET["categoria1"]) ? "" : "categoria/$ct1-$cat1->idcategoria1/"  ?>
						<?php echo empty($_GET["categoria2"]) ? "" : "categoria2/$ct2-$cat2->idcategoria2/" ?>
						<?php echo empty($_GET["buscar"]) ? "" : "buscar/$busqueda/" ?>
						<?php echo ($page+1); ?>">
	 ...
	 $query = $con->prepare('SELECT pp.*, p.nombre, p.precio, p.preciooferta, p.enoferta, ip.imagen, p.moneda
		...
		foreach($productosOferta as $pO)
		{
			...
			<div class="featItem">
				...
				<h3><a class="featItmName" href="/i/articulo/<?php echo $link."-".$pO->idproducto; ?>"><?php echo $pO->nombre ?></a></h3>
