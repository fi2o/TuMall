<?php 
	...
	{
		header('location: http://tumall.do/i/home');
	}*/
				...
				<div id="itemDetailsWrapper" class="group">
					<div class="itemPreviewsWrapper">
						<div class="itemMainPrev">
							...
							{
								?>
								<img src="/i/images/productos/<?php echo $imagenproducto[0]->imagen ?>" alt="<?php echo $producto->nombre ?>" />
								<?php 
							}
							...
							?>
								...
								<img src="/i/images/NoImage.png" alt="No image">
						...
						<div class="itemSmallerPrevWrapper">
							...
							foreach($imagenproducto as $iP)
							{
								...
								<div class="smallPrev <?php echo $smallImgCount == 0 ? "selectedItem" : "" ?>">
									<img <?php if($height > $width){  ?> width="<?php echo $baseDimm; ?>" height="<?php echo $modVal; ?>" style="top: -<?php echo $pos; ?>px;"  <?php }  else{ ?> height="<?php echo $baseDimm; ?>" width="<?php echo $modVal; ?>" style="left: -<?php echo $pos; ?>px;" <?php } ?> src="/i/images/productos/thumb150/<?php echo $iP->imagen ?>" alt="<?php echo $producto->nombre ?>" />
								</div>
								<?php
					...
					<div class="itemInfoWrapper">	
						...
						<div class="actionBar">
							...
							<a href="https://www.facebook.com/sharer.php?
								...
								&p[images][0]=http://www.tumall.do/i/images/productos/<?php echo $imagenproducto[0]->imagen; ?>
								&p[summary]="
							...
							<?php 
								...
								<script>
										$('.favWrap').click(function(){
											...
											$.ajax({
												type : "POST",
												url  : "/i/phpfn/favorite_products.php",
					...
					<div class="sellerInfoWrapper">
						<a class="sellerLogo" href="/i/<?php echo $link . "-" . $producto->idusuario ?>/">
							<?php 
							if(!empty($producto->imagen)){
							?>
								<img class="sellerLogoImg" src="/i/images/profile/cr/<?php echo $producto->imagen ?>" />
							<?php	
							}
							else{
								?>
								
								<img class="sellerLogoImg" src="/i/images/resources/storePNG-100.png" />
						...
						if($producto->estado == 'A')
						{
							...
							<div class="sellerType">
								<img src="/i/images/certVal.png" alt="Tienda certificada" title="Tienda Certificada (CFT)"  />
							</div>
							<?php
						}
						?>
						
						<a href="/i/<?php echo $link . "-" . $producto->idusuario ?>/" class="sellerDet sellerName"><?php echo $producto->nombretienda ?></a>
...
<div class="featItmWrap itemsFeatItmWrap">
	...
	foreach($productosOferta as $pO)
	{
		...
		<div class="featItem">
			...
			?>
			<a href="/i/articulo/<?php echo $link."-".$pO->idproducto; ?>" class="imgMask">
				<span class="maskMidle">
					<?php 
					if(empty($pO->imagen))
					{
					?>
						<img src="http://tumall.do/i/images/NoImage.png" alt="No image" title="No image">
					<?php 
					}
					else
					{
					?>
						<img src="/i/images/productos/thumb150/<?php echo $pO->imagen ?>" title="<?php echo $pO->nombre; ?>" alt="<?php echo $pO->nombre; ?>" >
					<?php 
					}
					?>
				</span>
			</a>
			<a class="featItmName" href="/i/articulo/<?php echo $link."-".$pO->idproducto; ?>"><?php echo $pO->nombre ?></a>
...
<div class="adsWrap">
 	...
 	$query = $con->prepare("SELECT * FROM `anunciopub`
		...
		?>
			...
			{
				?>
				<a class="top ads" href="<?php echo $anuncio[0]->link ?>"><img src="/i/images/publicidad/<?php echo $anuncio[0]->image ?>"></a>
				<?php 
			}
			?>
			...
			{
				?>
				<a class="mid ads" href="<?php echo $anuncio[1]->link ?>"><img src="/i/images/publicidad/<?php echo $anuncio[1]->image ?>"></a>
				<?php 
			}
			?>
			...
			{
				?>
				<a class="bottom ads" href="<?php echo $anuncio[2]->link ?>"><img src="/i/images/publicidad/<?php echo $anuncio[2]->image ?>"></a>
				<?php
			}
			?>
	...
	$('#itemDetailsWrapper').on('mouseenter', '.smallPrev', function(event){
		var hoverElem = $(this).children('img'),
			...
			if($(this).hasClass('alt')){
				$('.itemMainPrev img').attr('src', '/i/images/' + previewDir + '/' +previewSrc);
			}
			
			else{
				$('.itemMainPrev img').attr('src', '/i/images/productos/' +previewSrc);
			}
