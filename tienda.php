...
<?php 
			...
			(
			  	...
			  	foreach($categoria1 as $c1)
				{
					...
					<h3 class="resultsSubCategory">
						<a href="/i/<?php echo $tienda_url ?>-<?php echo $_GET["id"] ?>/1/<?php echo $link . "-" . $c1->idcategoria1 ?>/
					...
					if(!empty($categoria2))
					{
						...
						foreach($categoria2 as $c2)
						{
							...
							$link = preg_replace('/[^A-Za-z0-9\-]/', '', $link);
							?>
							<li><h4><a href="/i/<?php echo $tienda_url ?>-<?php echo $_GET["id"] ?>/2/<?php echo $link . "-" . $c2->idcategoria2 ?>/
				...
				?>
				<a class="view-more" href="/i/lista/">Ver otras categorías</a>
		</div>
	...
	<div class="resultsDisplayArea">
		<div class="storeDetailsWrap">
			...
			{
				?>
				url('/i/images/banners/cr/<?php echo $usuarioTienda->banner1 ?>');">
				<?php
			}	
			else{
				?>
					
				url('/i/images/resources/defaultBanner.png');">
				...
				<?php 
					...
					<script>
						$('.favWrap').click(function(){
							...
							$.ajax({
								type : "POST",
								url  : "/i/phpfn/favorite_store.php",
			...
			<div class="userProfilePic">
				...
				{
					?>
					/i/images/profile/cr/<?php echo $usuarioTienda->imagen ?>
					<?php	
				}
			...
			$('#searchInStore').submit(function( event )
				{
					...
					var url = '/i/<?php echo $tienda_url ?>-<?php echo $_GET["id"] ?>/';
					if(sear)
					{
						url = url + '' + sear + '/';
					}
		...
		<div class="storeDisplayContent">
			...
			foreach($productosTienda as $pT)
			{
				...
				<div class="itemWrap">
					...
					?>
					<a href="/i/articulo/<?php echo $link."-".$pT->idproducto; ?>" class="itemImg">
						<?php 
						if(empty($pT->imagen))
						{
						?>
						...
						{
						?>
							<img src="/i/images/productos/thumb400/<?php echo $pT->imagen ?>" alt="<?php echo $pT->nombre; ?>" title="<?php echo $pT->nombre; ?>">
						<?php 
						}
					...
					<div class="itemDescrip">
						<a href="/i/articulo/<?php echo $link."-".$pT->idproducto; ?>" class="itemListName"><?php echo $pT->nombre; ?></a>
						<br />
			...
			if($total_pages > 1)
				{
					...
					<?php
						...
						<a class="pageControl prevPage" href="/i/<?php echo $tienda_url ?>-<?php echo $_GET["id"] ?>/
					...
					for($i = 1; $i <= $total_pages; $i++)
					{
						if($page == $i)
						{
							...
							{
								?>
								<a class="toPage pageNumber" href="/i/<?php echo $tienda_url ?>-<?php echo $_GET["id"] ?>/
									<?php echo !empty($_GET["categoria1"]) ? "1/" . $cat1 . "-" . $_GET["categoria1"] . "/": "" ?>
									<?php echo !empty($_GET["categoria2"]) ? "2/" . $_GET["categoria2"] . "/": "" ?>
									<?php echo !empty($_GET["buscar"]) ? "" . $_GET["buscar"] . "/": "" ?>
									<?php echo "p/" . $i ?>
					...
					<?php
						...
						<a class="pageControl nextPage" href="/i/<?php echo $tienda_url ?>-<?php echo $_GET["id"] ?>/
									<?php echo !empty($_GET["categoria1"]) ? "1/" . $cat1 . "-" . $_GET["categoria1"] . "/": "" ?>
									<?php echo !empty($_GET["categoria2"]) ? "2/" . $_GET["categoria2"] . "/" : "" ?>
									<?php echo !empty($_GET["buscar"]) ? "" . $_GET["buscar"] . "/" : "" ?>
									<?php echo "p/" . ($page+1) ?>/">
