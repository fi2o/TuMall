...
$('.selectFormType').change(function(){
	...
	if (formType == 1)
	{
		$('.requestForm').attr("action", "/i/p/publicidad_func/update_cert.php");
	}
	else if (formType == 2)
	{
		$('.requestForm').attr("action", "/i/p/publicidad_func/update_adver.php");
	}
	else if (formType == 3)
	{
		$('.requestForm').attr("action", "/i/p/publicidad_func/update_offer.php");
	}
	
	if (formType >= 1){
		$.ajax({
			type : "POST",
			url : "/i/p/publicidad_func/get_form.php",
			data: {form : formType}
		}).done(function(html)
		{
