...
{
    ?>
    <script src="/i/ckeditor/ckeditor.js"></script>
    <?php
}
 else {
    ?>
    <script src="/i/simpleckeditor/ckeditor.js"></script>
    <?php
}
?>
...
if($producto->idusuario == $session["usuario"])
{
	...
	<div class="settWrap updateItemsWrap">
		...
		<form method="post" action="/i/p/upd_productos/upd_producto.php" class="itmDetailForm">
		...
		<div class="addImgWrap itmFormSec 3" data-id="3">
			...
			<div class="snglCharacWrap characDetail">
				<form id="imageform" method="post" enctype="multipart/form-data" action='/i/p/upd_productos/upd_img_predeterminadas.php'>
						...
						foreach($imagenProducto as $iP)
						{
							...
							<div class="uplddImgWrap">
								...
								<span class="itmImageMask imageDefaultMask">
									<img class="imageDefault itmImage" <?php if($height > $width){  ?> width="<?php echo $baseDimm; ?>" height="<?php echo $modVal; ?>" style="top: -<?php echo $pos; ?>px;"  <?php }  else{ ?> height="<?php echo $baseDimm; ?>" width="<?php echo $modVal; ?>" style="left: -<?php echo $pos; ?>px;" <?php } ?> src="/i/images/productos/thumb150/<?php echo $iP->imagen; ?>" >
								</span>
							</div>
							<?php
						}
				    ...
				    <div class="imagenes imgListWrap imgCaract defCharImgList newImgs">
				        ...
				        foreach($imagenes as $imgs)
				        {
				            ...
				            <div class="uplddImgWrap">
				                ...
				                <span class="itmImageMask imageDefaultMask">
				                	<img class="imageDefault itmImage" <?php if($height > $width){  ?> width="<?php echo $baseDimm; ?>" height="<?php echo $modVal; ?>" style="top: -<?php echo $pos; ?>px;"  <?php }  else{ ?> height="<?php echo $baseDimm; ?>" width="<?php echo $modVal; ?>" style="left: -<?php echo $pos; ?>px;" <?php } ?> src="/i/images/productos/thumb150/<?php echo $imgs->img_temporales ?>">
				                </span>
				            </div>
				            <?php
				        }
			...
			<div class="characsWrap">
				...
				{
					?>
					<form class="caracteristica xtraCharacDetail characDetail transTh" action="/i/p/upd_productos/upd_img_caract.php" method="post">
								...
								foreach($img_caract as $iC)
								{
									...
									<div class="uplddImgWrap">
										...
										<span class="itmImageMask">
											<img class="imageCaractProducto itmImage" <?php if($height > $width){  ?> width="<?php echo $baseDimm; ?>" height="<?php echo $modVal; ?>" style="top: -<?php echo $pos; ?>px;"  <?php }  else{ ?> height="<?php echo $baseDimm; ?>" width="<?php echo $modVal; ?>" style="left: -<?php echo $pos; ?>px;" <?php } ?> src="/i/images/c_prod/thumb150/<?php echo $iC->imagen ?>">
										</span>
									</div>
									<?php
								}
...
<script src="/i/Scripts/jquery.form.min.js"></script>
...
<script type="application/javascript"> 
	...
	$('.characsWrap').on('keyup','.caracteristica', function()
	{
		...
		$.ajax({
			type: "POST",
			url: "/i/p/upd_productos/upd_caracteristica.php",
	...
	$('.addImgWrap').on('click', '.itmImageMask', function(){
		...
		if($(this).hasClass('imageDefaultMask')){
			var imgUrl = ("/i/p/upd_productos/rem_prod_img.php");
		}
		
		else{
			var imgUrl = ("/i/p/upd_productos/rem_image_caract.php");
		}
	...
	$('.addCharacBtn').click(function(){
		...
		if(characCnt < 4){
			...
			//{
				$.ajax({
					type: "POST",
					url: "/i/p/upd_productos/add_caract_producto.php",
	...
	$('.characsWrap').on('click', '.removeCharacBtn', function()
	{
		...
		$.ajax({
			type: "POST",
			url: "/i/p/upd_productos/rem_caract.php",
	...
	function appendLoadFN(){
		targetDiv.parent().append('<span class="loadBlock"><img src="/i/images/LoadAnim.gif" /></span>');
	}
