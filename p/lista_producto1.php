...
if(!empty($_GET["id"]))
	...
	?>
		<a class="go-store-btn setting-extra-btn" href="/i/<?php echo $link . "-" . $usr2->idusuario ?>/">Ir a tienda</a>
	<?php 
	}
else
	{
	...
	?>
		<a class="go-store-btn setting-extra-btn" href="/i/<?php echo $link . "-" . $usuario->idusuario ?>/">Ir a tienda</a>
...
<div class="settWrap itmListWrap">
	<div class="listActionsWrap listSearchWrap">
		...
		{
			$i = $_GET["id"];
			$url = "href='http://tumall.do/i/administracion/usr/$i";
		}
		else
			{
				$url= "href='http://tumall.do/i/productos/lista/";
			}
		?>
...
{
	?>
	<p class="noContentMsg">Aún no has publicado artículos, <a href="http://tumall.do/i/productos/nuevo">agrega algunos</a>!</p>
	<?php
}
foreach($producto as $pd)
{
	...
	<div class="itemDetWrapp fluidTransTw <? if($oddCnt%2==1){ echo "odd";} echo $pd->activo == 1 ? "" : " inactive"; $oddCnt++  ?>">
    	...
    	<span class="itmImageMask itmListMask">
        	<img class="itmListImg itmImage" <?php if($height > $width){  ?> width="<?php echo $baseDimm; ?>" height="<?php echo $modVal; ?>" style="top: -<?php echo $pos; ?>px;"  <?php }  else{ ?> height="<?php echo $baseDimm; ?>" width="<?php echo $modVal; ?>" style="left: -<?php echo $pos; ?>px;" <?php } ?> src="/i/images/productos/thumb200/<?php echo $imagenProducto->imagen; ?>" >
        </span>
        <?php 
		}
		...
		<div class="itmListBtnWrapp">
			...
			{
				?>
				<a class="itmListBtn editBtn" href="/i/administracion/prod/<?php echo $pd->idproducto ?><?php echo empty($_GET["id"]) ? "" : "/" . $_GET["id"]; ?>">Editar</a>
				<?php
			}
			else
			{
				?>
				<a class="itmListBtn editBtn" href="/i/productos/lista/<?php echo $pd->idproducto ?><?php echo empty($_GET["id"]) ? "" : "/" . $_GET["id"]; ?>">Editar</a>
				<?php
			}
			?>
...
<script> //clears search terms in user list
	$('.removeSrchBtn').click(function()
	{
		...
		$.ajax({
			type : "POST",
			url : "/i/p/list_func/reset_srh.php"
		}).done(function(){
			location.reload();
		})
	})
	...
	$('.listSearchBtn').click(function()
	{
		...
		$.ajax({
			type : "POST",
			url : "/i/p/list_func/set_search.php",
			data : {search : srh}
		}).done(function(){
			location.reload();
		})
...
<script>
	$('.itmRemoveBtn').click(function()
	{
		if(confirm('Desear elminar este producto?'))
		{
			...
			$.ajax({
				type: "POST",
				url: "/i/p/list_func/rem_product.php",
...
<script>
	$('.itmStatBtn').click(function()
	{
		...
		$.ajax({
			type: "POST",
			url: "/i/p/list_func/change_status.php",
			data: {idproducto : producto}
		});
	});
</script>